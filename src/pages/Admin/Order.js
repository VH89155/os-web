...
import { Button, Input, Space, Table,Image,message,Progress } from 'antd';
...
const data = orderList.map((item,index)=>({...item,index})) || []
...
const columns = [
  ...
  },
  {
    ...
    sortDirections: ['descend', 'ascend'],
    render: (_,record) =>{
      if(record.status){return <Progress type="circle" percent={100} width={60} format={() => 'Done'} />}
      else return  <Progress type="circle" percent={100} width={60} status="exception" format={() => 'Chờ duyệt'}/>
...

  {
    ...
    render: (_, record) =>{ 
      ...
      <Space size="middle">
      <Button type="primary">Duyet</Button>
      <Link to={{
        pathname: "/admin/orderID",
        
      }}>
    <Button type="primary" onClick={()=>HandleShow(record.id)}>show</Button></Link>
